...
export function checkSwapGetEstimated(inputToken0, inputToken1, exactIn = true, chainId, library) {
  let {
  ...
  if (!inputToken0.symbol || !inputToken1.symbol) return false;
...
export async function swapGetEstimated(
  ...
  if (token0IsETH && token1Symbol === 'WETH') {
    ...
    let userHasSufficientBalance = userToken0Balance.gte(parseUnits(token0Amount, token0Decimal));
  ...
  // ETH <-> Non-WETH ERC20     OR     Non-WETH ERC20 <-> Non-WETH ERC20
    ...
    if (!token0IsETH) {
      let allowance = await getAllowance(token0Address, account, ROUTER_ADDRESS, library, account);
...
export async function swap(
  ...
  let status = await (async () => {
    ...
    if (token0IsETH && token1Symbol === 'WETH') {
      ...
      let wrappedAmount = BigNumber.from(parseUnits(token0Amount, token0Decimal)).toHexString();
