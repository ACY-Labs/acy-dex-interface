...
async function signOrApprove(
  ...
  let status = await (async () => {
    ...
    // ETH <-> Non-WETH ERC20     OR     Non-WETH ERC20 <-> Non-WETH ERC20
      ...
      let pairContract = usePairContract(pair.liquidityToken.address, library, account);
      ...
      let percentToRemove = new Percent(independentAmount.raw, liquidityValue.raw);
  ...
  let status = await (async () => {
    ...
    // ETH <-> Non-WETH ERC20     OR     Non-WETH ERC20 <-> Non-WETH ERC20
      ...
      if (!liquidityApproval) {
        ...
        return new ACYSwapErrorStatus('need approve for liquidityApproval');
      }
