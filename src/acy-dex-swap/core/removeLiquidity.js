...
export async function getPositionAndBalance(
  ...
  let { address: inToken0Address, symbol: inToken0Symbol, decimal: inToken0Decimal } = inToken0;
  let { address: inToken1Address, symbol: inToken1Symbol, decimal: inToken1Decimal } = inToken1;
  ...
  let status = await (async () => {
    ...
    // ETH <-> Non-WETH ERC20     OR     Non-WETH ERC20 <-> Non-WETH ERC20
      ...
      if (!liquidityApproval && signatureData === null) {
        ...
        return new CustomError('need approve for liquidityApproval');
      }
  ...
  return (
    <div>
      ...
      <Form>
        ...
        <Alert variant="info">{removeStatus}</Alert>
      </Form>
    </div>
  );
};
