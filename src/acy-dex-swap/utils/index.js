...
export async function addLiquidityGetEstimated(
  ...
  // ETH <-> Non-WETH ERC20     OR     Non-WETH ERC20 <-> Non-WETH ERC20
    ...
    if (exactIn) {
      dependentTokenAmount = pair.priceOf(token0).quote(new TokenAmount(token0, inputAmount.raw));

      parsed =
        token1 === ETHER ? CurrencyAmount.ether(dependentTokenAmount.raw) : dependentTokenAmount;
    } else {
      dependentTokenAmount = pair.priceOf(token1).quote(new TokenAmount(token1, inputAmount.raw));
...
export async function getUserRemoveLiquidityBalance(
  ...
  let { address: token0Address, symbol: token0Symbol, decimal: token0Decimal } = tokenA;
  let { address: token1Address, symbol: token1Symbol, decimal: token1Decimal } = tokenB;
