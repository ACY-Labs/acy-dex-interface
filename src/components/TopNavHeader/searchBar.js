...
export const SearchBar = props => {
  ...
  //         console.log("joy searchbar", data.data)
  //     })
  //         .catch(e => {
  //             console.log(e);
  //         });
  ...
  const onInput = useCallback(e => {
    ...
    setSearchCoinReturns(
      ...
      dataSourceCoin.filter(
        token => token.symbol.toUpperCase().includes(e.target.value.toUpperCase()) || token.name.toUpperCase().includes(e.target.value.toUpperCase())
      )
    );
