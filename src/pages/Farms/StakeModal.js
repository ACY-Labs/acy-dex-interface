    ...
    const checkStatusAndFinish = async () => {
      await library.getTransactionReceipt(status.hash).then(async receipt => {
        ...
        if (!receipt) {
          ...
          axios.get(
            // fetch valid pool list from remote
            `http://localhost:3001/api/updatePool?poolId=${poolId}`
          ...
          onClick={async () => {
            if (buttonText != "Done") {
              if (buttonText !== 'Approval required') {
                ...
                deposit(stakedTokenAddr, showStake, poolId, getLockDuration(), library, account, setButtonText, stakeCallback, setButtonStatus);
              } else {
